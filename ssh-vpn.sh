...
wget -q -O /etc/pam.d/common-password "https://raw.githubusercontent.com/limjianhui789/Blueblue-main/main/password"
...
wget -q -O /usr/local/bin/ws-stunnel "https://raw.githubusercontent.com/limjianhui789/Blueblue-main/main/ws-stunnel"
...
wget -q -O /usr/bin/badvpn-udpgw "https://raw.githubusercontent.com/limjianhui789/Blueblue-main/main/newudpgw"
...
# install dropbear (Debian 13 config format)
sleep 1
echo -e "[ ${green}INFO$NC ] Settings Dropbear"
# Debian 12+ uses DROPBEAR_OPTIONS instead of DROPBEAR_PORT/DROPBEAR_EXTRA_ARGS
cat > /etc/default/dropbear <<-DROPBEAR_CONF
NO_START=0
DROPBEAR_OPTIONS="-p 143 -p 109"
...
# Install Stunnel4 (from Debian repo instead of compiling from source)
sleep 1
echo -e "[ ${green}INFO$NC ] Installing Stunnel4 from repo"
apt install -y stunnel4 >/dev/null 2>&1

# Enable stunnel4
sed -i 's/ENABLED=0/ENABLED=1/g' /etc/default/stunnel4

# Download Config Stunnel4 (same port mappings as old stunnel5)
...
# Cleanup old stunnel5 artifacts if they exist
rm -fr /etc/stunnel5 >/dev/null 2>&1
rm -fr /etc/systemd/system/stunnel5.service >/dev/null 2>&1
rm -fr /etc/init.d/stunnel5 >/dev/null 2>&1
rm -f /usr/local/bin/stunnel5 >/dev/null 2>&1
rm -r -f /usr/local/share/doc/stunnel/ >/dev/null 2>&1
rm -r -f /usr/local/etc/stunnel/ >/dev/null 2>&1

# banner /etc/issue.net
sleep 1
echo -e "[ ${green}INFO$NC ] Settings banner"
wget -q -O /etc/issue.net "https://raw.githubusercontent.com/limjianhui789/Blueblue-main/main/issue.net"
chmod +x /etc/issue.net
echo "Banner /etc/issue.net" >> /etc/ssh/sshd_config
# Dropbear banner already set in /etc/default/dropbear config above
